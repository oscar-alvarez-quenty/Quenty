...
class EndpointTester:
    ...
    async def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            ...
            health_results = await self._test_all_health_endpoints()
            
            # Step 3: Parallel endpoint testing
            print("\n🔄 Step 3: Parallel Endpoint Testing...")
            endpoint_results = await self._test_all_endpoints_parallel()
    ...
    async def _test_all_health_endpoints(self) -> Dict[str, Any]:
        """Test health endpoints for all services."""
        health_tasks = []
        
        for service, base_url in self.base_urls.items():
            health_tasks.append(self._test_health_endpoint(service, base_url))
        
        health_results = await asyncio.gather(*health_tasks, return_exceptions=True)
        
        return {
            service: result if not isinstance(result, Exception) else {"error": str(result)}
            for service, result in zip(self.base_urls.keys(), health_results)
        }
    
    async def _test_health_endpoint(self, service: str, base_url: str) -> Dict[str, Any]:
    ...
    async def _test_all_endpoints_parallel(self) -> Dict[str, Any]:
        ...
        endpoint_definitions = {
            "auth": [
                ...
                {"method": "POST", "path": "/api/v1/users", "auth_required": True, "data": {
            ...
            if service in self.base_urls:
                ...
                for endpoint in endpoints:
                    ...
                    if isinstance(result, Exception):
                        result = {"error": str(result), "traceback": traceback.format_exc()}
                    organized_results[service].append(result)
                    task_index += 1
        
        return organized_results
    
    async def _test_single_endpoint(self, service: str, base_url: str, endpoint_def: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if method == "GET":
                async with self.session.get(url, headers=headers) as response:
                    ...
                    return await self._process_response(service, method, path, response, response_time)
            elif method == "POST":
    ...
    async def _process_response(self, service: str, method: str, path: str, response, response_time: float) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "response": response_data if len(str(response_data)) < 1000 else f"{str(response_data)[:1000]}...",
    ...
    def _generate_text_report(self, test_results: Dict[str, Any], docker_logs: Dict[str, Any]) -> str:
        ...
        if "health_checks" in test_results:
            for service, result in test_results["health_checks"].items():
                ...
                report.append(f"{emoji} {service.upper()}: {status}")
                if "error" in result:
...
async def main():
    ...
    try:
        ...
        test_results = await endpoint_tester.run_comprehensive_test()
